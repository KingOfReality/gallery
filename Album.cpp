...
void Album::setCreationDateNow()
{
	...
	oss << std::put_time(localtime(&now), "%d/%m/%Y");
...
std::ostream& operator<<(std::ostream& strOut, const Album& album)
{
	strOut << "[" << album.m_name << "] - created by user@"
		<< const_cast<Album&>(album).getOwnerId()<<" - created in "<<const_cast<Album&>(album).getCreationDate() << std::endl;
