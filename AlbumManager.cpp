...
void winapiShowPic(int program, Picture& pic)
{
	std::string cmd = "";
	STARTUPINFO startupInfo = { 0 };
	HANDLE hProccess = nullptr;
	...
	case 2:
		...
		break;
	default:
		throw MyException("Error: invalid application number.\n");
		break;
	}

	cmd += pic.getPath();
	BOOL success = CreateProcess(NULL, &cmd[0], NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &processInfo);
	if (success)
	{
		hProccess = processInfo.hProcess; // Set the global var of hProcess to the handle of the process we created so if ctrl c is pressed will be able to send a termination singal to the program
		WaitForSingleObject(processInfo.hProcess, INFINITE); // waiting for the program to close
		// closing program handles
		CloseHandle(processInfo.hProcess);
		CloseHandle(processInfo.hThread);
		hProccess = NULL; // setting the global var of hProcess to NULL in order that the singal handler function won't try to send a termination signal to a closed program and will perform regular ctrl c action
	}
	else
	{
		throw MyException("Error: issue occured while open of application.\n");
	}
}
void AlbumManager::showPicture()
{
	...
	std::string num = getInputFromConsole("What software you wanna use?\n1: painter\n2: iview\n");
	
	winapiShowPic(std::stoi(num.c_str()), pic);
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"" << m_dataAccess.countAlbumsOwnedOfUser(user)<<std::endl;
}
